...
from .modeling_xlnet import XLNetForSquenceClassification

form transformers import AdamW, get_linear_schedule_with_warmup

from .utils import glue_compute_metrics as compute_metrics
from .utils import glue_output_modes as output_modes
from .utils import glue_processors as processors
from transformers import glue_convert_examples_to_features as convert_examples_to_features

logger = logging.getLogger(__name__)

ALL_MODELS = sum((tuple(conf.pretrained_cnfig_archive_map.keys()) for conf in (BertConfig, XLNetConfig, XLMconfig,
                                                                               RobertaConfig, DistilBertConfig)), ())

MODEL_CLASSES = {
    'bert': (BertConfig, BertForSequenceClassification, BertTokenizer),
    'xlnet': (XLNetConfig, XLNetForSquenceClassification, XLNetTokenizer),
    'xlm': (XLMConfig, XLMForSequenceClassification, XLMTokenizer),
    'roberta': (RobertaConfig, RobertaForSequenceClassification, RobertaTokenizer),
    'distillbert': (DistilBertConfig, DistilBertForSequenceClassification, DistilBertTokenizer),
...
def train(args, train_dataset, model, tokenizer):
    ...
    if args.local_rank in [-1, 0]:
        ...
        train_sampler = RandomSampler(train_dataset) if args.local_rank == -1 else DistributedDSampler(train_dataset)
        train_dataloader = DataLoader(train_dataset, sampler=train_sampler, batch_size=args.train_batch_size)

        if args.max_steps > 0:
            t_total = args.max_steps
            args.num_train_epochs = args.max_steps // (len(train_dataloader) // args.gradient_accumulation_step) + 1
        else:
            t_total = len(train_dtaloader) // args.gradient_accumulation_steps * args.num_train_epochs
        ...
        logger.info(" Total train batch size (w. parallel, distributed & accumulation) = %d",
                    args.train_batch_size * args.gradient_accumulation_steps * (
                        torch.distributed.get_word_size() if args.local_rank != -1 else 1
                    ))
...
def evaluate(args, model, tokenizer, prefix=""):
    ...
    results = {}
    for eval_task, eval_outputs_dir in zip(eval_task_names, eval_outputs_dirs):
        if args.do_eval:
            mode = 'eval'
        if args.do_predict:
            mode = 'predict'
        eval_dataset = load_and_cache_example(args, eval_task, tokenizer, evaluate=True, mode=mode)
...
def load_and_cache_examples(args, task, tokenizer, evaluate=False, mode=None):
    ...
    if output_mode == 'classification':
        all_models = torch.tensor([f.label for f in features], dtype=torch.long)
...
def transformers_clf(args):
    ...
    if args.local_rank == -1 or args.no_cuda:
        ...
        torch.distributed.init process_group(backend='nccl')
        args.n_gpu = 1
    args.device = device
