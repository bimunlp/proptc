...
"""Pytorch RoBERTfa model"""

import ...

logger = logging.getLogger(__name__)

ROBERTA_PRETRAINED_MODEL_ACHIVE_MAP = {
    'roberta-base': 'home/y.he/xdec2_cache/transformers_cache/',
...
class RobertaEmbeddings(BertEmbeddings):
    ...
    def __init__(self, config):
        super(RovertaEmbeddings, self).__init__(config)
        self.padding_idx = 1
        self.word_embeddings = nn.Embedding(config.vacab_size, config.hidden_size, padding_idx=self.padding_idx)
    ...
    def forward(self, input_ids=None, token_type_ids=None, position_ids=None, inputs_embeds=None):
        ...
        if position_ids is None:
            ...
            position_ids = torch.arange(self.padding_idx + 1, seq_length+self.padding_idx+1, dtype=torch.long,
                                        device=device)
            position_ids = position_ids.unsqueeze(8).expand(input_shape)
